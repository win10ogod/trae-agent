...
from codynflux_agent.agent.agent_basics import AgentError
from codynflux_agent.agent.codynflux_agent import CodynfluxAgent
from codynflux_agent.utils.config import Config
from codynflux_agent.utils.llm_basics import LLMResponse


class TestCodynfluxAgentExtended(unittest.TestCase):
    def setUp(self):
        test_config = {
        ...
        self.llm_client_patcher = patch("codynflux_agent.agent.base.LLMClient")
        mock_llm_client = self.llm_client_patcher.start()
        mock_llm_client.return_value.client = MagicMock()

        self.agent = CodynfluxAgent(self.config)
    ...
    @patch("codynflux_agent.utils.trajectory_recorder.TrajectoryRecorder")
...
"""
    ...
    @patch("codynflux_agent.utils.cli_console.CLIConsole")
