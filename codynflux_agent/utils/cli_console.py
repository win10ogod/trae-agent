...
class CLIConsole:
    ...
    def print_task_details(
        ...
        mode_info = []
        if dtdd_mode:
            mode_info.append("DTDD (Document-Driven Development)")
        if multi_agent_mode:
            mode_info.append("Multi-Agent Coordination")
        
        mode_display = f"[bold]Mode:[/bold] {', '.join(mode_info)}" if mode_info else ""
        
        details = f"""[bold]Task:[/bold] {task}
